...
int main(int argc, char **argv) {
	...
	// ファイルから、データ(y, n)を読み込む
	load_data(argc, argv, &K, &y, &n);

	// データyをデバイスのバッファへ転送する
	CUDA_CALL(cudaMalloc((void **)&dev_y, K * sizeof(int)));
	CUDA_CALL(cudaMemcpy(dev_y, y, K * sizeof(int), cudaMemcpyHostToDevice));

	// データnをデバイスのバッファへ転送する
	CUDA_CALL(cudaMalloc((void **)&dev_n, K * sizeof(float)));
	CUDA_CALL(cudaMemcpy(dev_n, n, K * sizeof(float), cudaMemcpyHostToDevice));

	// theta用のバッファをデバイス側に確保する。* Allocate space for theta and log_theta on device and host */
	CUDA_CALL(cudaMalloc((void **)&dev_theta, K * sizeof(float)));
	CUDA_CALL(cudaMalloc((void **)&dev_log_theta, K * sizeof(float)));

	// MCMCのステート用のバッファをデバイス側に確保する
...
__host__ float sample_a(float a, float b, int K, float sum_logs) {
	...
	log_acceptance_ratio = (proposal - a) * sum_logs + K * (proposal - a) * log(b) - K * (lgamma(proposal) - lgamma(a));
...
__device__ float rgamma(curandState *state, int id, float a, float b){
	...
	while(1) {
		...
		/* Accept the sample under the following condition. Otherwise repeat loop. */
		if (log(U) < 0.5 * pow(Y,2) + d * (1 - v + log(v))) return d * v / b;
	}
}
...
__global__ void setup_kernel(curandState *state, unsigned int seed, int K) {
	...
	if(id < K) curand_init(seed, id, 0, &state[id]);
}